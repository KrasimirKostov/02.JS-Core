function solve() {
  ...
  inputArr.forEach(element => {
    ...
    if (match) {
      insertValidEelement(`Name: ${match[1]}`);
      insertValidEelement(`Phone Number: ${match[2]}`);
      insertValidEelement(`Email: ${match[3]}`);


    } else {
      resultElement.appendChild(invalidDataParagraph.cloneNode(true));//chrez true kopira textContenta; bez true kopira elementa
    }
    resultElement.appendChild(dashElement.cloneNode(true));
  });


  function insertValidEelement(text){
